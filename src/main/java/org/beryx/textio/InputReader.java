...
public abstract class InputReader<T, B extends InputReader<T, B>> {
	...
	protected String getDefaultErrorMessage(String sVal) {
		StringBuilder errBuilder = new StringBuilder(Translate.tr("Invalid value"));
		if (valueListMode) {
			errBuilder.append(" ").append(Translate.tr("in the comma-separated list"));
			if (itemName != null)
				errBuilder.append(" ").append(Translate.tr("of")).append(" '").append(itemName).append("'");
			if (sVal != null && !sVal.isEmpty())
				errBuilder.append(": ").append(sVal);
		} else {
			if (itemName != null)
				errBuilder.append(" ").append(Translate.tr("for")).append(" '").append(itemName).append("'");
		}
		errBuilder.append('.');
	...
	protected final List<String> getErrorMessages(String s) {
		if (parseErrorMessagesProvider != null)
			return parseErrorMessagesProvider.getErrorMessages(s, itemName);
...
mainLoop:
			while (true) {
				...
				if (sValues.length == 0 && defaultValue != null)
		...
		while (true) {
			...
			try {
				...
				switch (data.getAction()) {
					case CONTINUE:
						logger.error("ReadInterruptionException with action CONTINUE."); // no break here: handle it as RESTART
					case RESTART:
	...
	private T getValueFromStringOrIndex(String sVal, TextTerminal<?> textTerminal) {
		if (possibleValues == null || !numberedPossibleValues)
			return getValueFromString(sVal, textTerminal);
		else
			return getValueFromIndex(sVal, textTerminal);
	}
		...
		if (errMessages == null) {
			...
			textTerminal.executeWithPropertiesPrefix(PROPS_PREFIX_ERROR_MESSAGE, t -> {
				...
				if (inlinePossibleValues) {
					String options = possibleValues.stream().map(val -> "'" + valueFormatter.apply(val) + "'").collect(Collectors.joining(", "));
					t.println(" " + Translate.tr("Please enter one of") + ": " + options + ".");
				} else {
					t.println(" " + Translate.tr("Please enter one of the displayed values") + ".");
				}
			});
			textTerminal.println();
		} else {
	...
	private T getValueFromIndex(String sVal, TextTerminal<?> textTerminal) {
		...
		textTerminal.executeWithPropertiesPrefix(PROPS_PREFIX_ERROR_MESSAGE, t -> {
			if (invalidIndexErrorMessagesProvider != null) {
				...
				textTerminal.println(" " + Translate.tr("Enter a value between 1 and") + " " + possibleValues.size() + ".");
			}
		});
		textTerminal.println();
		return null;
	...
	protected void checkConfiguration() throws java.lang.IllegalArgumentException {
		...
		for (ValueChecker<T> checker : valueCheckers) {
			...
			if (defaultValue != null) {
				...
				if (errors != null)
					throw new IllegalArgumentException("Invalid default value: " + valueFormatter.apply(defaultValue) + ".\n" + errors);
			}
			if (possibleValues != null) {
				for (T val : possibleValues) {
					errors = checker.getErrorMessages(val, itemName);
					if (errors != null)
						throw new IllegalArgumentException("Invalid entry in the list of possible values: " + valueFormatter.apply(val) + ".\n" + errors);
				}
			}
		}
	}
	...
	protected void printPrompt(List<String> prompt, TextTerminal<?> textTerminal) {
		...
		if (possibleValues == null) {
			...
			if (inlinePossibleValues) {
				String sValues = IntStream.range(0, optionCount).mapToObj(i -> possibleValues.get(i)).map(option -> {
				...
				textTerminal.print(valueListMode ? Translate.tr("Enter your choices as comma-separated values") + ": " :
					Translate.tr("Enter your choice") + ": ");
			}
		}
	}
	...
	public static <T> ValueChecker<List<T>> nonEmptyListChecker() {
		return (list, propName) -> {
			if (list == null || list.isEmpty())
				return Collections.singletonList(Translate.tr("Expected at least one element") + ".");
			else
				return null;
		};
	}
	...
	public static <T> ValueChecker<List<T>> noDuplicatesChecker() {
		return (list, propName) -> {
			...
			if (valueSet.size() < list.size())
				return Collections.singletonList(Translate.tr("Duplicate values are not allowed") + ".");
			return null;
		};
	}
}
