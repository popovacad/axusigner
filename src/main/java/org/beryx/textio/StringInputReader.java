...
public class StringInputReader extends InputReader<String, StringInputReader> {
	...
	protected List<String> getLengthValidationErrors(String s) {
		...
		IntFunction<String> chr = l -> l + " " + ((l > 1) ? Translate.tr("characters") : Translate.tr("character")) + ".";
		if (minLength > 0 && minLength > len)
			return Collections.singletonList(Translate.tr("Expected a string with at least") + " " + chr.apply(minLength));
		if (maxLength > 0 && maxLength < len)
			return Collections.singletonList(Translate.tr("Expected a string with at most") + " " + chr.apply(maxLength));
		return null;
	}

	protected List<String> getPatternValidationErrors(String s) {
		if ((pattern != null) && !pattern.matcher(s).matches())
			return Collections.singletonList(Translate.tr("Expected format") + ": " + pattern.pattern());
		return null;
	}
}
