...
public class SwingTextTerminal extends AbstractTextTerminal<SwingTextTerminal> {
	...
	private static class StyleData {
		...
		public String getStyleName() {
			return Stream.of(id(color), id(bgColor), id(bold), id(italic), id(underline), id(strikeThrough), id(subscript), id(superscript), fontFamily, "" + fontSize).collect(Collectors.joining("|"));
		}
	...
	public SwingTextTerminal() {
		...
		try {
			...
			logger.error("Cannot set look and feel", e);
		}
	...
	private boolean plainReplaceText(String message, StyleData styleData) {
		...
		if (overwriteOffset < 0)
			return plainInsertMessage(message, styleData);
	...
	public static Optional<Color> getColor(String colorName) {
		...
		try {
			return Optional.of(Color.decode(colorName));
		} catch (Exception e) {
